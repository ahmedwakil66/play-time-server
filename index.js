...
async function run() {
    try {
        ...
        const animalToyCollection = client.db('playTime').collection('animalToys');
        ...
        app.get('/animal-toys', async (req, res) => {
            ...
            //pagination. send 10 toys data by default
                ...
                let options = {};
                if (req.query.sortByPrice && (req.query.sortByPrice == 1 || req.query.sortByPrice == -1)) {
                    options = {
                        sort: { price: req.query.sortByPrice }
                    }
                } else if (req.query.sortByQuantity && (req.query.sortByQuantity == 1 || req.query.sortByQuantity == -1)) {
                    options = {
                        sort: { availableQuantity: req.query.sortByQuantity }
                    }
                }

                const result = await animalToyCollection.find({}, options).skip(skip).limit(limit).toArray();
                res.send(result);
            }
        })
